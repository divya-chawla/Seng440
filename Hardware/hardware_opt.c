/*
    ...
    SENG 440 
    Audio Compression Hardware Optimised C file
    By - 
...
int main (int argc, char **argv) {
    if (argc < 2) {
        printf("Please input a valid .wav file \n");
        exit(1);
    }
    ...
    readWaveFile(new_fp);
    //parse data sample
    readWaveFileDataSamples(new_fp);
...
void readWaveFileDataSamples(FILE *new_fp) {
    ...
    if (header.format_type == 1){ 
        ...
        compression(num_samples);
        decompression(new_fp, num_samples, size_of_each_sample);
    }
    //if not PCM 
    else
    {
...
void compression(long num_samples){
    start = clock();
    compressDataSamples(num_samples);
    stop = clock();
    //optimised
...
void decompression(FILE *new_fp, long num_samples, long size_of_each_sample){
    start = clock();
    decompressDataSamples(num_samples);
    stop = clock();
...
void compressDataSamples(long num_samples) {
    ...
    for (i = 0; i < num_samples; i++) {
    #pragma HLS unroll factor=10
        register int sample = (sample_data[i] >> 2);
        compressed_sample_data[i] = codeword_compression(sample);
    }
